...
**Logcat has a real offset, but the buffer wraps fast.** `logcat -T '<MM-DD HH:MM:SS.mmm>'` genuinely returns only lines at or after that timestamp — a durable watermark, unlike `UsageStatsManager`'s coarse buckets. But the main buffer on a busy phone can wrap well inside this app's usual 30-minute collector cadence, so a logcat collector needs its own, shorter cadence (5 minutes in the shipped `LogcatCollector`) or it will silently miss lines between cycles. Filter client-side by keyword rather than parsing structure — tags and formats aren't stable across OEM builds — and store matches aggregated per cycle, not one row per line, per the storage-growth note below.

**`READ_LOGS` is confirmed non-functional for cross-process visibility on this device.** This is more severe than the `BatteryStatsManager` correction above: there, the fix was "grant both things together." Here, both things were granted and it still doesn't work, with no known complementary unlock. Verified: `READ_LOGS` declared in the manifest, granted via `scripts/provision.sh`, `dumpsys package` shows `granted=true`, and the running process's `/proc/<pid>/status` shows gid `1007` (`log`), matching `platform.xml`'s `READ_LOGS` → `gid="log"` mapping — so the grant is real, not just PackageManager bookkeeping. Despite that, an on-device run logged 110 cycles over 9 hours, and every line ever read carried this app's own PID (`pids=1` throughout), against 10 distinct PIDs visible to the shell UID over the same window. The one "match" the collector ever produced before this was fixed was a false positive: androidx.work's own `SystemJobScheduler` boilerplate, logged by this app's own WorkManager instance, coincidentally contains the substring `jobscheduler`. The likely explanation is platform hardening beyond what the documented permission model implies (AOSP `logd` restricting real access to privileged/system apps regardless of group membership, Samsung SELinux, or both) — but the mechanism doesn't matter as much as the result: **this signal is not obtainable via the documented T2 grant on this device.** The collector infrastructure (watermark, cadence, dedup, keyword filtering) is sound and confirmed working. Self-line exclusion is narrower than it sounds: it only ever filters the *currently-running* process's PID, so a line from an earlier incarnation of this same app (observed during testing, across repeated reinstalls) can still pass — a `matches` row is evidence worth reading, not proof the source wasn't still this app. None of this changes the core finding: it has no genuine cross-process data to read. Keeping it running costs real storage for a permanent `read_status` tripwire (~290 rows/day of `ok ... pids=1`) in exchange for detecting if a future OS update changes this.

**These grants survive reboot but are wiped on reinstall.** Put them in `scripts/provision.sh` and call it from the install task, or you will silently lose T2 on the next build. (`app/build.gradle.kts` wires `provisionT2` as `finalizedBy` on `installDebug` so this can't be forgotten.)
